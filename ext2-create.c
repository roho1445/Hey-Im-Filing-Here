...
void write_inode_table(int fd) {
	...
	hello_world_file_inode.i_mode = EXT2_S_IFREG
	                              ...
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH;
	hello_world_file_inode.i_uid = 1000;
	hello_world_file_inode.i_size = strlen("Hello world\n");
	hello_world_file_inode.i_atime = current_time;
	hello_world_file_inode.i_ctime = current_time;
	hello_world_file_inode.i_mtime = current_time;
	...
	struct ext2_inode hello_symbolic_inode = {0};
	hello_symbolic_inode.i_mode = EXT2_S_IFLNK
	                              | EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH;
...
void write_root_dir_block(int fd)
{
	...
	struct ext2_dir_entry hello_world_file = {0};
	dir_entry_set(hello_world_file, HELLO_WORLD_INO, "hello-world");
	dir_entry_write(hello_world_file, fd);
	...
	struct ext2_dir_entry hello_link = {0};
	dir_entry_set(hello_link, HELLO_INO, "hello");
	dir_entry_write(hello_link, fd);

	bytes_remaining -= hello_link.rec_len;
